...
export default function AdminDocketTemplatesPage() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-gray-50/50 dark:bg-slate-900 transition-colors">
      <div className="flex items-center justify-between px-6 py-4 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 shrink-0">
        <div>
          <h1 className="text-lg font-semibold text-gray-900 dark:text-slate-100">Docket Templates</h1>
          <p className="text-sm text-gray-500 dark:text-slate-400">Manage furniture types and their dynamic fields</p>
        </div>
        <button
          onClick={handleAddNew}
          className="flex items-center gap-2 px-4 py-2 text-sm text-white font-medium rounded-lg transition-colors hover:opacity-90"
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {templates.map(tpl => (
            <div key={tpl.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm">
              <div className="flex justify-between items-start mb-4">
                <h3 className="font-semibold text-gray-900 dark:text-slate-100 text-lg">{tpl.name}</h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
              ...
              <button onClick={() => setEditingTemplate(null)} className="text-gray-400 hover:text-gray-600 dark:hover:text-slate-300">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-gray-50 dark:bg-slate-700/50 border border-gray-200 dark:border-slate-600 rounded-lg focus:outline-none focus:border-purple-500 text-gray-900 dark:text-slate-100"
                />
              </div>
                ...
                <div className="space-y-3">
                  {editingTemplate.fields.map((field, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      />
                      <button onClick={() => handleRemoveField(idx)} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  ))}
            ...
            <div className="px-6 py-4 border-t border-gray-100 dark:border-slate-700 flex justify-end gap-3 bg-gray-50/50 dark:bg-slate-800/80">
              <button onClick={() => setEditingTemplate(null)} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-slate-300 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-lg transition-colors">
                Cancel
              </button>
              <button onClick={handleSave} disabled={loading} className="px-6 py-2 text-sm font-medium text-white rounded-lg transition-colors flex items-center gap-2 hover:opacity-90 disabled:opacity-50" style={{ background: '#875a7b' }}>
